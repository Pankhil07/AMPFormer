...
class Trainer:
    ...
    def freeze_initial_layers(self, freeze_up_to_layer: int):
        """Freeze first N transformer layers while keeping embeddings trainable"""
        print("\n=== Freezing Layers ===")
        
        for name, param in self.model.named_parameters():
            # Handle compiled model prefixes
            original_name = name.replace("_orig_mod.", "")
            
            # 1. Force UNFREEZE all embeddings
            if "token_embedding" in original_name:
                param.requires_grad = True
                print(f"Embedding kept trainable: {name}")
                continue
                
            # 2. Freeze specified transformer layers
            if "layers." in original_name:
                try:
                    # Extract layer index from "layers.{n}"
                    layer_index = int(original_name.split("layers.")[1].split(".")[0])
    ...
    def train(
        ...
        if self._epoch >= self.config.max_epochs:
            return
        #self.freeze_initial_layers(freeze_up_to_layer=3)
