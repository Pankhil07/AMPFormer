...
class Hyformer(LLAMABackbone):
        ...
        if task == 'lm':
            ...
            if target is not None:
                if self.prediction_task_type == 'classification':
                    ...
                    if valid_logits.dim() == 1 or valid_logits.size(-1) == 1:
                    ...
                    #flat_pred   = logits.view(-1)
                    #flat_target = target.view(-1)

                    #valid = flat_target != nan_target_idx
                    #if valid.sum() == 0:
                    #    return None

                    #pred = flat_pred[valid]
                    #true = flat_target[valid]

                    # -------- imbalance weighting ----------------------------------
                    # bucketise true values into the same bins used for training stats
                    #bin_idx = torch.bucketize(true, self.imb_edges[1:-1], right=True)
                    #samp_w  = self.imb_w[bin_idx].to(true.device)        # (n_valid,)

                    # weighted MSE:  Σ w_i (y-ŷ)² / Σ w_i
                    #mse = F.mse_loss(pred, true, reduction="none")
                    #loss = (mse * samp_w).sum() / samp_w.sum()
                    #return loss
                    ...
                    # mask out only the -1 placeholders
                    valid = flat_target != nan_target_idx
                    num_valid = valid.sum().item()

                    if num_valid == 0:
                        # no real targets this batch → skip loss
                        return None

                    pred = flat_logits[valid]
                    true = flat_target[valid]

                    # now compute MSE only over real targets
                    return F.mse_loss(pred, true, reduction=loss_fn_reduction)
                    return F.mse_loss(
                        logits.view(-1, ), 
                        target.view(-1, ), 
                        reduction=loss_fn_reduction
    ...
    # ---------------------------------------------------------------
    #  NOTE: put this method inside your model class  (replaces old one)
    # ---------------------------------------------------------------
    @torch.inference_mode()
    def generate_1(
        self,
        ...
        *,
        ...
        use_cache: bool = True,
        return_last_hidden_state: bool = False,     # NEW FLAG
        device: torch.device | None = None,
    ) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        """
        Parameters
        ----------
        prefix_input_ids : (B, L0) LongTensor
            Prompt to condition on.
        num_tokens_to_generate : int
            Number of new tokens after the prefix.
        eos_token_id / pad_token_id : int
            Special-token ids.
        temperature, top_k, top_p : float / int / None
            Sampling hyper-parameters.
        use_cache : bool
            Enable KV-cache for fast generation.
        return_last_hidden_state : bool
            If True, also return the last-token hidden state of each sequence
            (shape  (B, hidden_dim) ).
        device : torch.device | None
            Force tensors to this device (defaults to prefix_input_ids.device).

        Returns
        -------
        If return_last_hidden_state is False
            seq_ids : (B, L) LongTensor
        else
            (seq_ids, h_last)  where h_last is (B, hidden_dim) FloatTensor
        """
        # ------------ setup ----------------------------------------------------
        if device is None:
            device = prefix_input_ids.device
        batch_size, prefix_len = prefix_input_ids.shape
        gen_len = prefix_len + num_tokens_to_generate

        # (1) init output tensor
        ...
        # (3) optional KV-cache
        if use_cache:
            self.init_cache(batch_size, gen_len)   # your own helper

        # ------------ autoregressive loop -------------------------------------
        for pos in range(prefix_len, gen_len):
            # choose prompt slice depending on cache
            if use_cache and pos > prefix_len:
                prompt = output_ids[:, [pos]]           # only the last token
            else:
                prompt = output_ids[:, :pos]            # full prefix so far

            next_tok = self._generate_single_token(     # your helper
                prefix_input_ids=prompt,
            ...
            # mask with PAD if EOS already hit
            next_tok = torch.where(eos_flags, pad_token_id, next_tok)

            output_ids[:, pos] = next_tok
            eos_flags |= (next_tok == eos_token_id)
        ...
        seqs = output_ids[:, :gen_len]   # final sequences  (B, L)

        # ------------ optional second pass for hidden state -------------------
        if not return_last_hidden_state:
            return seqs

        # build attention mask for the forward pass
        attn_mask = seqs.ne(pad_token_id).long()

        last_h = self(                          # single batched forward-pass
            seqs,
            task="lm",                          # or "prediction" if that’s the head
            attention_mask=attn_mask,
            return_loss=False,
        )["embeddings"][:, -1, :]               # (B, hidden_dim)
